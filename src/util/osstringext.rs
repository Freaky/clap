...
impl OsStrOps<'_> {
    ...
    pub fn arg_starts_with<S: AsRef<str>>(&self, s: S) -> bool {
        match &self {
            #[cfg(not(unix))]
            OsStrOps::Str(v) => s.as_ref().starts_with(v),
            #[cfg(unix)]
            OsStrOps::Bytes(v) => s.as_ref().as_bytes().starts_with(v),
            #[cfg(windows)]
            OsStrOps::Wide(v) => s.as_ref().encode_utf16().collect::<Vec<u16>>().starts_with(v),
        }
    }
